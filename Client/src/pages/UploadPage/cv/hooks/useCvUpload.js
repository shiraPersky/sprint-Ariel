...
const useCvUpload = () => {
  ...
  useEffect(() => {
    if (selectedFile) {
      //If a file is selected, it automatically starts simulating the upload
      simulateUpload();
    }
  }, [selectedFile]);

  const handleFileChange = (e) => {//when the user choose file
  ...
  const handleDrop = (e) => {//when the user drag a file into the screen
    e.preventDefault();//prevent automatically open of the file
  ...
  const simulateUpload = () => {
    //This creates a fake upload animation by updating the progress every 150ms
    let progress = 0;
  ...
  const resetUpload = () => {
    //Clears all state and resets the hook to its initial state
