...
function generateICSContentSimple({ time, description }) {
    ...
    const start = format(new Date(time), "yyyyMMdd'T'HHmmss'Z'");
    const end = format(new Date(new Date(time).getTime() + 60 * 60 * 1000), "yyyyMMdd'T'HHmmss'Z'");
...
BEGIN:VEVENT
UID:event-${time}@community-app.com
DTSTAMP:${start}
DTSTART:${start}
DTEND:${end}
SUMMARY:${description}
